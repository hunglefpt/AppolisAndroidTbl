/**
 * Name: $RCSfile: AcCycleCountLocation.java,v $
...
import com.appolis.adapter.CycleCountCurrentLocationAdapter;
...
/**
 * @author CongLT
 * Display Cycle count location screen
 */
public class AcCycleCountLocation extends Activity implements OnClickListener,
		OnItemClickListener {
	...
	private CycleCountCurrentLocationAdapter cycleCountCurrentAdapter;
	// data variable
	...
	private LoadCycleCountListAsyn loadLocationAsyn;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		objectCycleCount = (ObjectCycleCount) intent
				.getSerializableExtra("ObjectCycleCount");
		bin = (ObjectInstanceRealTimeBin) intent
				.getSerializableExtra("BinCycleCount");
		languagePrefs = new LanguagePreferences(getApplicationContext());
		initLayout();
		setText();
		loadLocationAsyn = new LoadCycleCountListAsyn(this);
		loadLocationAsyn.execute();
	}
	...
	private void initLayout() {
		...
		// setData
		tvCurrentLocaction.setText(bin.get_binNumber());
	...
	/**
	 * Process data Cycle count list from api
	 * @author hoangnh11
	 */
	private class LoadCycleCountListAsyn extends AsyncTask<Void, Void, Integer> {
			...
			if (!isCancelled()) {
				try {
					...
					netParameter[0] = new NetParameter("cyclecountInstanceID",
							String.valueOf(objectCycleCount
									.get_cycleCountInstanceID()));
					netParameter[1] = new NetParameter("binNumber",URLEncoder.encode(bin.get_binNumber().trim(), GlobalParams.UTF_8));
		...
		protected void onPostExecute(Integer result) {
			...
			if (!isCancelled()) {
				if (result == 0) {
					// new Adapter within Current Location
					cycleCountCurrentAdapter = new CycleCountCurrentLocationAdapter(
							getApplicationContext(), binList, bin, objectCycleCount, AcCycleCountLocation.this);
					...
					Utilities.dialogShow(errorMss, AcCycleCountLocation.this);
				}
			} else {
				Utilities.dialogShow(errorMss, AcCycleCountLocation.this);
			}
	...
	private void startActivityBinPath(ObjectCountCycleCurrent currentSelected) {
		...
		Intent intentCycleBin = new Intent(AcCycleCountLocation.this,
		...
		startActivityForResult(intentCycleBin,
	 ...
	 * @param currentSelected
	 */
	private void startActivityCycleAdjustment(
			ObjectCountCycleCurrent currentSelected) {
		path = bin.get_binNumber();
		Intent intentCycleAdjustment = new Intent(AcCycleCountLocation.this,
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.lin_buton_scan:
				Intent intentScan = new Intent(AcCycleCountLocation.this,
			...
			case R.id.lin_layout_Cycle_Adjustment:
				dialogOptions.dismiss();
				Intent intent = new Intent(AcCycleCountLocation.this, AcCycleAdjustmentOption.class);
				intent.putExtra("BinCycleCount", bin);
				ObjectBinList objectBinList = new ObjectBinList();
				objectBinList.setBinList(binList);
				intent.putExtra("isLocationScreen", true);
			...
			case R.id.lin_layout_cycle_zero:
				...
				dialogConfirmZeroCount(mesConfirm, AcCycleCountLocation.this);
				break;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case GlobalParams.AC_CYCLE_COUNT_ADJUSMENT_ACTIVITY:
			if (RESULT_OK == resultCode || resultCode == RESULT_CANCELED) {
				new LoadCycleCountListAsyn(AcCycleCountLocation.this).execute();
			}
			break;
	...
	private void processScanData(String barcode) {
		ProcessScanDataAsyn mLoadDataTask = new ProcessScanDataAsyn(
				AcCycleCountLocation.this, barcode);
			...
			if (!isCancelled()) {
				if (checkBarcodeInList()) {
					...
					case 6: // If Item Number, UOM or ItemIdentification
						new LoadItemAsyn(AcCycleCountLocation.this)
								.execute(barcodes);
						break;
		...
		protected void onPostExecute(Integer result) {
			...
			if (!isCancelled()) {
				if (result == 0) {
					...
					if (StringUtils
							.isNotBlank(currentSelected.get_itemNumber())) {
				...
				if (result == 0) {
					...
					if (countCycleCurrents.size() > 0) {
						...
						// only has different Lot in first list
							...
							dialogShowMessage(contentMessage, AcCycleCountLocation.this);
						}
					}
	...
	public void dialogConfirmZeroCount(String message, Activity activity) {
		...
		btDialogYes.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				dialog.dismiss();
				new ZeroCountAsyn(AcCycleCountLocation.this).execute();
			}
		});
